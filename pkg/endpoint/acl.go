...
func PrincipalExtractor[ID pkg.IDLike](extractor policy.FromPrincipalExtractor[ID]) auth.UserIDExtractor[ID] {
...
func ContextValueExtractor[ID pkg.IDLike](key any) auth.UserIDExtractor[ID] {
	return func(ctx context.Context) (ID, error) {
		val := ctx.Value(key)
		if val == nil {
			return *new(ID), policy.ErrPermission
...
func PrincipalOwnerOrAdminAccessControl(table string, ownerField string) *auth.OwnerOrAdminAccessControl[uuid.UUID] {
	owner := auth.NewOwnerAccessControl(ownerField, table, PrincipalExtractor(policy.UserUUIDExtractor))

	return auth.NewOwnerOrAdminAccessControl(owner, func(ctx context.Context) (bool, error) {
		p, ok := policy.GetPrincipal(ctx)
		if !ok || p == nil {
			return false, nil
		}
