...
import (
	...
	"github.com/smilemakc/mbkit/application/endpoint"
...
func (ga *GinAdapter) Mount(base string, endpoints ...endpoint.Endpoint) {
	if ga.Write == nil {
...
func DefaultGinErrWriter(c *gin.Context, err error) {
	status, payload := endpoint.ClassifyHTTPError(err)
